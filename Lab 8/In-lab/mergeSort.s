...
mergeSort:
	...
	cmp rsi, rdx		; recursion base case
	jge endMergeSort

	push rsi
	push rdx

	add rdx, rsi		; find middle index by finding average of being and end
	shr rdx, 1		; divide by 2 by shifting right by one bit
	push rdx
	call mergeSort		; rescursive call on first half

	pop rsi			; restore values
	pop rdx
	inc rsi			; increment middle index
	push rdx
	call mergeSort		; recursive call on second half

	pop rdx			; restore values again
	pop rsi
	mov rcx, rdx		; copy end index to parameter 4 for merge
	add rdx, rsi
	shr rdx, 1
	call merge

...
int main(int argc, char *argv[])
{
  ...
  hashTable dictionary(getNextPrime(2 * size));
  hashTable prefix(getNextPrime(2 * size));
	  ...
	  for(int d = 0; d < 8; d++)
	    {
	      for(int l = 3; l < 22; l++)
		{
		  word = getWordInGrid(r, c, d, l, rows, cols);

		  
		  if(l == 3)
		    {
		      if(prefix.contains(word) == false)
			break;
		    }
		  		  

		  if(dictionary.contains(word) == true && word.length() > 2)
		    {
		      count++;
			...
			direction = "NW";
		      ...
		      cout << direction << "(" << r << ", " << c << "):       " << word << endl;
		    }

		  if(getWordInGrid(r, c, d, l + 1, rows, cols) == word)
		    break;
		}
	    }
	}
  ...
  //cout << (int)t.getTime() << endl;   //outputs the number of seconds taken; this line is being used for averageTime.sh
  
  return 0;
}

//Everything below this line was given by the assignment
//Wasn't really sure if I was supposed to #include the files but clang++ was giving me issues about having multiple main methods
//Lord help us
